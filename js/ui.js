...
const UI = {
  ...
  // Chat functionality delegated to ChatUI (see ui-chat.js)
  ...
  // Camera functionality delegated to CameraUI (see ui-camera.js)
  ...
  // Entity functionality delegated to EntityUI (see ui-entities.js)
...
UI.openMeetingCapture = function() {
  ...
  // Note: MeetingCapture handles all saving internally with note_type: 'meeting'
  // The callback is just for logging - no additional save needed
  if (typeof MeetingCapture !== 'undefined') {
    MeetingCapture.init(container, {
      onEnhanceComplete: (data) => {
        console.log('[UI] Meeting callback:', data.saved ? 'saved' : 'enhanced', data.noteId || '(no id yet)');
        // MeetingCapture handles save, toast, close, refresh, and sync internally
      }
    });
  }
