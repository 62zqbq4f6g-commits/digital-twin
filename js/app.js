...
const App = {
   ...
   * @param {string} options.note_type - Note type (e.g., 'meeting', 'decision')
   * @returns {Promise<Object>} Saved note with ID
   */
  async processNote(rawText, inputType, options = {}) {
    ...
    try {
      ...
      const note = this.buildNoteObject(text, inputType, classification, extracted, refined, analysis, options);
   ...
   * @param {Object} options - Optional parameters (note_type, etc.)
   * @returns {Object} Complete note object
   */
  buildNoteObject(rawText, inputType, classification, extracted, refined, analysis = null, options = {}) {
    const now = new Date();
    ...
    // Add note_type if provided (for meetings, decisions, etc.)
    if (options.note_type) {
      noteObject.note_type = options.note_type;
      noteObject.type = options.note_type;
    }
