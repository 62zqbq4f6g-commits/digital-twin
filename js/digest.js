...
const Digest = {
  ...
  async generate() {
    ...
    try {
      ...
      // Call digest API with user_id for contradiction detection
  ...
  showDigest(digest, weekStart, weekEnd, noteCount) {
    ...
    content.innerHTML = `
      ...
      ${digest.evolutions && digest.evolutions.length > 0 ? `
        <div class="digest-section digest-evolutions">
          <h3 class="digest-section-title">Evolutions This Week</h3>
          <p class="digest-subtext">Changes in your thinking or sentiment:</p>
          <ul class="digest-list">
            ${digest.evolutions.map(e => `<li>${e}</li>`).join('')}
          </ul>
        </div>
      ` : ''}
  ...
  formatDigestForCopy() {
    ...
    if (d.evolutions && d.evolutions.length > 0) {
      text += `---\n\n`;
      text += `## Evolutions This Week\n\n`;
      text += `*Changes in your thinking or sentiment:*\n\n`;
      d.evolutions.forEach(e => text += `• ${e}\n`);
      text += `\n`;
    }
