...
import { convertProfileToFacts, convertKeyPersonToFacts } from './profile-converter.js';
 ...
 * Converts structured profile fields to SPO triples
 */
async function extractFromProfile(userId, data, options = {}) {
  const facts = convertProfileToFacts(data);

  if (facts.length === 0) {
    console.log('[INPUT-ROUTER] No facts to extract from profile');
    return null;
  }

  // Store facts directly (no AI needed for structured data)
  const results = await storeProfileFacts(userId, facts, options);
 ...
 * Converts user choices to facts
 */
async function extractFromSettings(userId, data, options = {}) {
  ...
  if (data.context_mode) {
    facts.push({
      predicate: 'prefers_context_mode',
      object: data.context_mode,
      confidence: 1.0
    });
  }
  ...
  if (data.tier) {
    facts.push({
      predicate: 'privacy_preference',
      object: data.tier === 'byok' ? 'maximum_privacy' : 'balanced',
      confidence: 1.0
    });
  }
  ...
  const results = await storeProfileFacts(userId, facts, options);
  console.log(`[INPUT-ROUTER] Settings extraction complete:`, results);
  return results;
}

/**
 * Store profile facts directly (no AI extraction needed)
 */
async function storeProfileFacts(userId, facts, options = {}) {
  ...
  // Get or create the "user" entity (represents the user themselves)
  let { data: userEntity } = await supabase
    .from('user_entities')
    .select('id')
    .eq('user_id', userId)
    .eq('name', 'Self')
  ...
  if (!userEntity) {
    console.error('[INPUT-ROUTER] Failed to create user entity');
    return { factsCreated: 0, factsUpdated: 0 };
  }
  ...
  return { factsCreated, factsUpdated };
}
