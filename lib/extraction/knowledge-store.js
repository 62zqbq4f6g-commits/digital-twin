...
export async function storeExtraction(userId, extraction, options = {}) {
  ...
  try {
    // 1. Store entities (with source tracking for cascade delete)
    for (const entity of extraction.entities || []) {
      const result = await upsertEntity(supabase, userId, entity, sourceType, sourceId);
      if (result) {
    ...
    // 2. Store topics (with source tracking for cascade delete)
    for (const topic of extraction.topics || []) {
      const result = await upsertTopic(supabase, userId, topic, sourceType, sourceId);
      if (result) {
        if (result.created) {
 ...
 * @param {string} sourceType - Source type (note, mirror, meeting)
 * @param {string} sourceId - Source record ID for cascade delete tracking
 */
async function upsertEntity(supabase, userId, entity, sourceType, sourceId = null) {
  try {
    ...
    // Create new entity with source tracking for cascade delete
    const { data: newEntity, error } = await supabase
      ...
      .insert({
        ...
        source_note_id: sourceId,  // Track source for cascade soft-delete
        status: 'active'
      })
      .select('id')
      .single();
 ...
 * @param {string} sourceId - Source record ID for cascade delete tracking
 */
async function upsertTopic(supabase, userId, topic, sourceType = 'note', sourceId = null) {
  try {
    ...
    // Create new topic with source tracking
...
/**
 * Upsert a fact (relationship) with bi-temporal tracking
 *
 * For single-value predicates (works_at, lives_in, etc.), the database
 * trigger will automatically detect contradictions and:
 * 1. Invalidate the old fact with invalidation_reason = 'contradiction'
 * 2. Link the new fact to the old one via previous_version_id
 * 3. Increment the version number
 */
async function upsertFact(supabase, userId, entityId, rel, sourceType, sourceId) {
  try {
    // Check for existing CURRENT fact with same predicate and object
    // (We only update if it's the exact same fact, not a contradiction)
    const { data: existing } = await supabase
      .from('entity_facts')
    ...
    // Create new fact with bi-temporal metadata
    // The database trigger (handle_fact_contradiction) will:
    // - Detect if this contradicts an existing fact
    // - Auto-invalidate the old fact
    // - Set previous_version_id and version
    ...
    const { data: newFact, error } = await supabase
      ...
      .insert({
        ...
        last_mentioned: now,
        // Bi-temporal fields
        valid_from: now,           // When the fact became true
        is_current: true,          // This is the current version
        version: 1,                // Will be updated by trigger if contradiction
        status: 'active'
      })
      .select('id, version, previous_version_id')
      .single();
    ...
    // Log if contradiction was detected (version > 1 means trigger fired)
    if (newFact.version > 1) {
      console.log('[KNOWLEDGE-STORE] Contradiction detected, created version', newFact.version);
    }

    return { id: newFact.id, created: true, version: newFact.version };
